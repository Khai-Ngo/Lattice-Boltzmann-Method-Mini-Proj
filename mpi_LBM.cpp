...
class LatticeBoltzmann{
    ...
    public:
        ...
        void latticeReset(){
            // no need to parallelise lattice resetting for next test of MPI Proc num LOL
            for (int j = 0; j<lat_size;j++){
...
int main(int argc, char* argv[]){
    ...
    if (argc!=6){
        ...
        std::cout<<"<.exe><maxProcNo><length><width><timeStepsNumber><saveFlag>\n";
        return 0;
    }

    int maxProcNo = std::stoi(argv[1]);
    int l = std::stoi(argv[2]);
    int w = std::stoi(argv[3]);
    int time = std::stoi(argv[4]);
    int saveFlag = std::stoi(argv[5]);
    double *results = new double[maxProcNo];
    ...
    for (int i = 1; i< maxProcNo+1;i++){
        results[i-1]=mySim->simulate(u0,time,i);
        if (i == saveFlag){
        // just one set of output for a random threadNum for sanity check
        std::string inf3 = "rho_02u0_"+std::to_string(l)+"x"+std::to_string(w)+"_"+std::to_string(time)+"sec_"+std::to_string(i)+"processes.txt";
        std::string inf4 = "xVel_02u0_"+std::to_string(l)+"x"+std::to_string(w)+"_"+std::to_string(time)+"sec_"+std::to_string(i)+"processes.txt";
        mySim->exportRho(inf3);
        mySim->exportxVel(inf4);
        }
        mySim->latticeReset();
        std::cout<<"Proc. no = "<<i<<". Took "+std::to_string(results[i-1])+" seconds\n";
    }
    // save results to .txt file
    std::string fname = "LBM_"+std::to_string(l)+"x"+std::to_string(w)+"_"+std::to_string(time)+"steps_vs_nProcesses.txt";
